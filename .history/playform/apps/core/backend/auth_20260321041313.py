import os
ㄋfrom datetime import datetime, timedelta
...
def dashboard(current_user: User = Depends(get_current_user), session=Depends(get_session)):
    """Protected dashboard — returns profile stats for the logged-in user."""
    fs = get_firestore()
    if fs:
        profiles_col = fs.collection("profiles")
        profiles_docs = profiles_col.stream()
        profiles = []
        ...
        return {"user": current_user.username, "total_profiles": total_profiles, "profiles": profiles}
