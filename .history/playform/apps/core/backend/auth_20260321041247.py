...
def list_games_public(session=Depends(get_session)):
    """Return all games (public, no auth required)."""
    fs = get_firestore()
    if fs:
        games_col = fs.collection("games")
        games = []
        for doc in games_col.stream():
            d = doc.to_dict()
            d["id"] = d.get("id", doc.id)
            games.append(d)
        return games
...
async def create_game_public(
    game_in: GameIn,
    background_tasks: BackgroundTasks,
    request: Request,
    session=Depends(get_session)
):
    """Create a new game entry and trigger n8n workflow."""
    ...
    background_tasks.add_task(trigger_n8n_webhook, game_in.name, username)

    fs = get_firestore()
    if fs:
        doc_ref = fs.collection("games").add({"name": game_in.name})
        try:
            doc_id = doc_ref[1].id if doc_ref and len(doc_ref) > 1 else None
        except Exception:
            doc_id = None
        return {"created": {"id": doc_id, "name": game_in.name}}

    game = Game(name=game_in.name)
    session.add(game)
    session.commit()
    session.refresh(game)
    return {"created": game.dict()}


@router.delete("/api/games/{game_id}")
def delete_game(game_id: int, session=Depends(get_session)):
    """Delete a game by ID."""
